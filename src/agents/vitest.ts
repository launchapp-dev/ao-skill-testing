  ...
  private extractFunctions(file: TsMorphSourceFile): AnalyzedFunction[] {
    ...
    file.getVariableDeclarations().forEach(varDecl => {
      ...
      if (initializer && 
        ...
        functions.push({
          name,
          parameters: (initializer as any).getParameters?.()?.map((p: any) => ({
            name: p.getName(),
            type: p.getType()?.getText(),
            optional: p.isOptional?.(),
            defaultValue: p.getInitializer()?.getText()
          })) || [],
          returnType: (initializer as any).getReturnType?.()?.getText?.() || undefined,
          isAsync: (initializer as any).isAsync?.() || false,
          isExported: true,
...
class VitestTestGenerator {
   ...
   */
  generate(sourceFile: SourceFile): TestGenerationResult {
   ...
   */
  private generateFunctionTests(analysis: AnalyzedModule, functions: AnalyzedFunction[]): GeneratedTest {
    const lines: string[] = [];
  ...
  private generateTestsForFunction(func: AnalyzedFunction): { code: string; testCases: GeneratedTest['testCases'] } {
    ...
    if (this.options.includeEdgeCases) {
      ...
      const stringParams = func.parameters.filter(p => 
        p.type?.toLowerCase().includes('string')
      );
      if (stringParams.length > 0) {
        ...
        const emptyArgs = func.parameters.map(p => 
          p.type?.toLowerCase().includes('string') ? "''" : this.generateSampleValue(p.type)
        ).join(', ');
      ...
      const numberParams = func.parameters.filter(p => 
        p.type?.toLowerCase().includes('number')
      );
      if (numberParams.length > 0) {
        ...
        const zeroArgs = func.parameters.map(p => 
          p.type?.toLowerCase().includes('number') ? '0' : this.generateSampleValue(p.type)
        ).join(', ');
   ...
   */
  private generateTestsForMethod(cls: AnalyzedClass, method: AnalyzedFunction): { code: string; testCases: GeneratedTest['testCases'] } {
    const lines: string[] = [];
    const testCases: GeneratedTest['testCases'] = [];
    ...
    if (this.options.includeEdgeCases) {
      ...
      const edgeArgs = method.parameters.map(p => 
        p.type?.toLowerCase().includes('string') ? "''" : 
        p.type?.toLowerCase().includes('number') ? '0' : 'null'
      ).join(', ');
   ...
   */
  private generateHeader(analysis: AnalyzedModule): string[] {
    const lines: string[] = [];
   ...
   */
  private generateTestArgs(func: AnalyzedFunction): string {
    return func.parameters.map(p => 
      p.defaultValue || this.generateSampleValue(p.type)
    ).join(', ');
  }
   ...
   */
  private generateConstructorArgs(cls: AnalyzedClass): string {
    return cls.constructorParams.map(p => 
  ...
  private generateSampleValue(type?: string): string {
    ...
    if (lower.includes('number')) return '42';
    if (lower.includes('boolean')) return 'true';
    if (lower.includes('array')) return '[]';
    if (lower.includes('object')) return '{}';
    if (lower.includes('function')) return '() => {}';
    if (lower.includes('promise')) return 'Promise.resolve(undefined)';
    if (lower.includes('date')) return 'new Date()';
    if (lower.includes('map')) return 'new Map()';
    if (lower.includes('set')) return 'new Set()';
  ...
  private inferJsType(tsType: string): string {
    ...
    if (lower.includes('void') || lower.includes('undefined')) return 'undefined';
    if (lower.includes('null')) return 'object';
    if (lower.includes('promise')) return 'object';
   ...
   */
  private getTestPath(sourcePath: string, suffix = ''): string {
...
export class VitestUnitTestAgent {
  private generator: VitestTestGenerator;
  private options: Required<TestGenerationOptions>;
