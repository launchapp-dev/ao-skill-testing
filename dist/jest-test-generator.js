...
export class JestTestGenerator extends TestGenerator {
    ...
    generate(analysis) {
        ...
        result.metadata.totalTests = result.tests.reduce((sum, test) => sum + test.testCases.length, 0);
        return result;
    }
    /**
    ...
    generateImports(analysis) {
        ...
        const typeImports = analysis.imports.filter(imp => imp.specifiers.some(spec => analysis.functions.some(f => f.parameters.some(p => p.type?.includes(spec)))));
    ...
    generateTestsForFunction(func) {
        ...
        if (this.jestOptions.includeAutoMocks) {
            const callbackParams = func.parameters.filter(p => p.type?.toLowerCase().includes('function') ||
...
`;
     ...
     */
    generateJestSetup(customMatchers = []) {
        const lines = [];
     ...
     */
    generateMockArguments(func, mockParam) {
...
//# sourceMappingURL=jest-test-generator.js.map