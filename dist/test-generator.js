...
export class TestGenerator {
    ...
    generateTestsForFunction(func) {
        ...
        if (this.options.includeTypeTests && func.returnType) {
            ...
            lines.push(`      const result = ${func.name}(${this.generateSampleArguments(func)});`);
            lines.push(`      // Expected type: ${func.returnType}`);
        ...
        if (this.options.includeEdgeCases) {
            func.parameters.forEach(param => {
                if (!param.optional) {
                    ...
                    lines.push(`      // Test with boundary value for ${param.name}: ${param.type || 'unknown type'}`);
            ...
            lines.push(`      // Test null/undefined handling`);
    ...
    generateClassTests(analysis, cls) {
        ...
        // Test properties
        cls.properties.filter(p => p.visibility === 'public').forEach(prop => {
            testCases.push({
                name: `${cls.name}.${prop.name} - property`,
                type: 'unit',
                targetClass: cls.name
            });
            lines.push(`  it('should have ${prop.name} property', () => {`);
            lines.push(`    const instance = new ${cls.name}(${this.generateSampleConstructorArgs(cls)});`);
            lines.push(`    expect(instance.${prop.name}).toBeDefined();`);
            lines.push(`  });`);
            lines.push('');
        });
        lines.push('});');
